...
public class Validator {
    ...
    private class ValidationResult {
        ...
        public List<Validator.FD> invalidFDs = new ArrayList<>();
        public List<IntegerPair> comparisonSuggestions = new ArrayList<>();
        public void add(Validator.ValidationResult other) {
    ...
    private class ValidationTask implements Callable<Validator.ValidationResult> {
        ...
        public Validator.ValidationResult call2() throws Exception {
            Validator.ValidationResult result = new Validator.ValidationResult();
            ...
            if (Validator.this.level == 0) {
                ...
                for (int rhsAttr = rhs.nextSetBit(0); rhsAttr >= 0; rhsAttr = rhs.nextSetBit(rhsAttr + 1)) {
                    ...
                    if (!Validator.this.plis.get(rhsAttr).isConstant(Validator.this.numRecords)) {
                        ...
                        result.invalidFDs.add(new Validator.FD(lhs, rhsAttr));
                    }
        ...
        public Validator.ValidationResult call() throws Exception {
            Validator.ValidationResult result = new Validator.ValidationResult();
    ...
    private Validator.ValidationResult validateSequential(List<FDTreeElementLhsPair> currentLevel) throws AlgorithmExecutionException {
        Validator.ValidationResult validationResult = new Validator.ValidationResult();
    ...
    private Validator.ValidationResult validateParallel(List<FDTreeElementLhsPair> currentLevel) throws AlgorithmExecutionException {
        Validator.ValidationResult validationResult = new Validator.ValidationResult();

        List<Future<Validator.ValidationResult>> futures = new ArrayList<>();
        for (FDTreeElementLhsPair elementLhsPair : currentLevel) {
            Validator.ValidationTask task = new Validator.ValidationTask(elementLhsPair);
            futures.add(this.executor.submit(task));
        }

        for (Future<Validator.ValidationResult> future : futures) {
            try {
    ...
    public List<IntegerPair> validatePositiveCover() throws AlgorithmExecutionException {
        ...
        while (!currentLevel.isEmpty()) {
            ...
            Validator.ValidationResult validationResult = (this.executor == null) ? this.validateSequential(currentLevel) : this.validateParallel(currentLevel);
            ...
            int candidates = 0;
            for (Validator.FD invalidFD : validationResult.invalidFDs) {
